...
from datasets import *    #原来是datasets
...
import genotypes
from NeuEvo_model import NetworkCIFAR
...
val_acc_history = []    # 记录验证准确率
CONFUSION_MATRIX_EPOCH =  50 # 触发混淆矩阵的epoch
CONFUSION_MATRIX_EPOCH1 = 598
CONFUSION_MATRIX_EPOCH2 =  600
...
parser.add_argument('--arch', default='dvsc10_quadratic_standard', type=str)
...
try:
    ...
    from apex_parallel import DistributedDataParallel as ApexDDP
    from apex_parallel import convert_syncbn_model
...
def main():
    ...
    try:  # train the model
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            #train_acc_history.append(train_metrics.get('top1', 0))  # 假设train_metrics包含top1指标
            val_acc_history.append(eval_metrics.get('top1', 0))     # 假设eval_metrics包含top1指标
    
            if epoch == CONFUSION_MATRIX_EPOCH or epoch == CONFUSION_MATRIX_EPOCH1 or epoch == CONFUSION_MATRIX_EPOCH2:
              # print("[DEBUG] Train Acc History:", train_acc_history)
               print("[DEBUG] Val Acc History:", val_acc_history)
               print(f"[DEBUG] 验证集长度: {len(val_acc_history)}")
               x = range(1, len(val_acc_history) + 1)
               plt.figure(figsize=(10, 6))
               #plt.plot(x, train_acc_history, 'b-', label='Train Acc')
               plt.plot(x, val_acc_history, 'r-', label='Val Acc')
               plt.xlabel('Epoch')
               plt.ylabel('Accuracy')
               plt.legend()
    ...
    print("Available models:", timm.models.list_models(pretrained=False)[:10])  # 只显示前10个
