...
from NeuEvo_operations import *
from genotypes import Genotype
from utils import drop_path
from timm.models import register_model
from NeuEvo_base_node import *
from braincog.base.connection.layer import *
from NeuEvo_model_zoo_base_module import BaseModule


def neuron_cluster_membrane_potential(neuron_outputs, k_bilinear=0.0):
    """
    计算神经元簇的膜电位，包含双线性项
...
class MlpCell(BaseModule):
    ...
    def _forward_once(self, s0, s1, drop_prob):
        ...
        for i in range(self._steps):
            ...
            # 使用新的神经元簇膜电位计算方式
            neuron_outputs = [h1, h2]
            s = neuron_cluster_membrane_potential(neuron_outputs, self.k_bilinear)
            if self.back_connection:
                if i != 0:
...
class Cell(nn.Module):
    def __init__(self, genotype, C_prev_prev, C_prev, C, reduction, reduction_prev, act_fun, back_connection, k_bilinear=0.0):
...
class DCOCell(nn.Module):
    def __init__(self, genotype, C_prev_prev, C_prev, C, reduction, reduction_prev, act_fun, k_bilinear=0.15):
    ...
    def forward(self, s0, s1, drop_prob):
        ...
        for to_i, ops in self._ops.items():
            ...
            # 使用新的神经元簇膜电位计算方式
            out = neuron_cluster_membrane_potential(h, self.k_bilinear)
            if self.training and drop_prob > 0:
        ...
        for i in range(layers):
            ...
            if parse_method == 'darts':
                cell = Cell(genotype, C_prev_prev, C_prev, C_curr,
                            ...
                            act_fun=self.act_fun, back_connection=self.back_connection, k_bilinear=self.k_bilinear)
            else:
                cell = DCOCell(genotype, C_prev_prev, C_prev, C_curr,
                               reduction, reduction_prev, act_fun=self.act_fun, k_bilinear=self.k_bilinear)
